...
$(function() {
    ...
    var priceTotal = $('.price-total span:nth-child(2)').text();

    $('.products-item').each(function () {
       var parentId = $(this).closest('.swiper-slide').index().toString();
       var thisId = $(this).index().toString();
       $(this).attr('data-id', parentId + '-' + thisId);
    });

    $('.products-item-buttons').each(function () {
        $(this).append('<button class="qty-less" type="button"><span>-</span></button><button class="qty-more" type="button"><span>+</span></button>');
        var numInput = $(this).find('input');
        var inputVal = numInput.val();
        var qtyLess = $(this).find('.qty-less');
        var qtyMore = $(this).find('.qty-more');
        var productId = $(this).closest('.products-item').attr('data-id');
        var productName = $(this).closest('.products-item').find('.products-item-name').text();
        var qtyVal = $(this).closest('.products-item').find('.products-item-qty span:first-child');
        var productPrice = parseInt($(this).closest('.products-item').find('.products-item-price span').text());
        qtyVal.text(inputVal);
        qtyLess.on('click', function () {
            var productId = $(this).closest('.products-item').attr('data-id');
            if(parseInt(inputVal) !== 0){
                inputVal = inputVal - 1;
                priceTotal = parseInt(priceTotal) - productPrice;
                $('.price-total span:nth-child(2)').text(priceTotal)
                $('.cart-item[data-id=' + productId +'] .cart-item-qty span').text(inputVal);
            }
            if(parseInt(inputVal) < 1){
                $(this).closest('.products-item').find('.products-item-qty').removeClass('visible');
                $('.cart-item[data-id=' + productId + ']').remove();
            }else{
                $('.cart-item[data-id=' + productId +'] .cart-item-price span').text(productPrice*inputVal);
            }
            if(!parseInt(priceTotal) > 0){
                $('.price-panel').removeClass('visible');
            }
        ...
        qtyMore.on('click', function () {
            ...
            numInput.val(inputVal);
            qtyVal.text(inputVal);
            $(this).closest('.products-item').find('.products-item-qty').addClass('visible');
            $('.price-panel').addClass('visible');
            if(inputVal === 1){
                $('.cart-list').append('' +
                    '<div class="cart-item" data-id="' + productId +'">' +
                    '<div class="cart-item-cont">' +
                    '<div class="cart-item-name">' + productName +'</div>' +
                    '<div class="cart-item-qty">' + 'x' +'<span>' + inputVal +'</span></div>' +
                    '<div class="cart-item-price">' +
                    '<span>' + productPrice + '</span> p.' +
                    '</div>' +
                    '</div>' +
                    '<div class="cart-item-buttons">' +
                    '<button type="button" class="cart-item-less"><span>-</span></button>' +
                    '<button type="button" class="cart-item-more"><span>+</span></button>' +
                    '</div>' +
                    '</div>' +
                    '');
            }
    ...
    $(document).on('click', '.cart-item button', function () {
        ...
        if($(this).hasClass('cart-item-less')){
            $('.products-item[data-id='+ productId + '] .qty-less').trigger('click');
        }else{
            $('.products-item[data-id='+ productId + '] .qty-more').trigger('click');
        }
    });
